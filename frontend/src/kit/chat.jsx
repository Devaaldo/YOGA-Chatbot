...
(function () {
	...
	const REGIONS = [
		{ keys: ["bantul"], name: "Bantul" },
		{ keys: ["sleman"], name: "Sleman" },
		{ keys: ["gunungkidul", "gunung kidul"], name: "Gunungkidul" },
		{ keys: ["kulon progo", "kulonprogo"], name: "Kulon Progo" },
		{
			keys: ["kota", "yogyakarta", "jogja", "malioboro"],
			name: "Kota Yogyakarta",
		},
	];
	const topRated = (list, n = 3) =>
	...
	function respond(text, lang) {
		...
		const t = norm(text);
		if (
			/\b(halo|hai|hi|hello|hey|selamat)\b/.test(t) &&
			t.split(" ").length <= 3
		)
			return {
				reply: L(
					lang,
					"Halo! Saya YOGA, asisten wisata Yogyakarta. Mau cari wisata apa hari ini?",
					"Hi! I'm YOGA, your Yogyakarta travel assistant. What would you like to find today?",
				),
				places: [],
			};
		if (/\binfo\b|tentang|about|detail/.test(t)) {
			...
			if (hit)
				return {
					reply: L(lang, `Ini ${hit.name}:`, `Here's ${hit.name}:`),
					places: [hit],
				};
		}
		if (/lokasi|location|di mana|dimana|where/.test(t)) {
			...
			if (hit)
				return {
					reply: L(
						lang,
						`${hit.name} ada di ${hit.regency}.`,
						`${hit.name} is in ${hit.regency}.`,
					),
					places: [hit],
				};
		}
		...
		if (/pantai|beach/.test(t)) {
			...
			if (list.length)
				return {
					reply: L(
						lang,
						`Pantai terbaik${region ? " di " + region.name : ""}:`,
						`Top beaches${region ? " in " + region.name : ""}:`,
					),
					places: topRated(list),
				};
		}
		if (/candi|temple/.test(t)) {
			...
			if (list.length)
				return {
					reply: L(
						lang,
						"Candi & situs bersejarah pilihan:",
						"Top temples & historic sites:",
					),
					places: topRated(list),
				};
		}
		if (/murah|hemat|budget|cheap|tiket/.test(t)) {
			const list = [...PLACES]
				.filter((p) => p.priceWeekday != null && p.priceWeekday > 0)
				.sort((a, b) => a.priceWeekday - b.priceWeekday);
			return {
				reply: L(
					lang,
					"Wisata dengan tiket paling ramah kantong:",
					"The most budget-friendly spots:",
				),
				places: list.slice(0, 3),
			};
		}
		if (/rating|terbaik|top|bagus|best/.test(t))
			return {
				reply: L(
					lang,
					"Wisata dengan rating terbaik di Jogja:",
					"The highest-rated places in Jogja:",
				),
				places: topRated(PLACES),
			};
		if (region || /rekomendasi|recommend|wisata|saran/.test(t)) {
			...
			return {
				reply: L(
					lang,
					`Rekomendasi wisata${region ? " di " + region.name : " terbaik"}:`,
					`Recommended places${region ? " in " + region.name : ""}:`,
				),
				places: topRated(list.length ? list : PLACES),
			};
		}
		return {
			reply: L(
				lang,
				"Maaf, saya kurang paham. Coba tanya rekomendasi, pantai di suatu daerah, tiket murah, atau info sebuah tempat.",
				"Sorry, I didn't quite get that. Try recommendations, beaches in a region, budget tickets, or info about a place.",
			),
			places: [],
		};
	}
	...
	function ChatThread({
			...
			(text) => {
				...
				fetch(API + "/api/chat", {
					...
					.then((r) => {
						...
						setMsgs((m) => [
							...m,
							{ from: "yoga", text: r.reply, places: r.places || [] },
						]);
					})
					.catch(() => {
		...
		React.useEffect(() => {
			if (askSignal && askSignal.text) send(askSignal.text);
		}, [askSignal && askSignal.n]);
		React.useEffect(() => {
			if (bodyRef.current)
		...
		const cardWidth =
			variant === "page" ? { width: "100%", maxWidth: 460 } : { width: "88%" };

		return (
			<React.Fragment>
				<div
					className={
						"jjk-chat__body" + (variant === "page" ? " jjk-chatpage__body" : "")
					}
					ref={bodyRef}
				>
					{msgs.map((m, i) => (
						<React.Fragment key={i}>
							<Bubble from={m.from} time={m.time}>
								{m.text}
							</Bubble>
							{m.places && m.places.length
								? m.places.map((p) => (
										<div
											key={p.id}
											style={{ alignSelf: "flex-start", ...cardWidth }}
										>
											<DestinationCard
												place={p}
												variant="mini"
				...
				{msgs.length <= 1 ? (
					<div className="jjk-chat__qr">
						{QUICK(lang).map((q, i) => (
							<QuickReply key={i} label={q} onClick={() => send(q)} />
						))}
					</div>
				) : null}
				<form
					className="jjk-chat__foot"
					onSubmit={(e) => {
						e.preventDefault();
						send(input);
					}}
				>
					<input
						className="jjk-chat__input"
						value={input}
						onChange={(e) => setInput(e.target.value)}
						placeholder={L(lang, "Tulis pesan…", "Type a message…")}
						aria-label={L(lang, "Tulis pesan", "Type a message")}
					/>
					<IconBtn
						variant="soft"
		...
		return (
			<div className="jjk-chat" role="dialog" aria-label="YOGA chat">
				<div className="jjk-chat__head">
					...
					<div>
						<div className="jjk-chat__name">YOGA</div>
						<div className="jjk-chat__status">
							{L(
								lang,
					...
					<span className="jjk-chat__close">
						<IconBtn
							variant="ghost"
							size="sm"
			...
			<div className="jjk-chatpage">
				<div className="jjk-container jjk-chatpage__inner">
					<header className="jjk-chatpage__hero">
						...
						<div>
							<span className="jjk-chatpage__badge">
								{L(lang, "Asisten AI", "AI assistant")}
							</span>
							<h1 className="jjk-chatpage__title">
								{L(lang, "Ngobrol dengan ", "Chat with ")}
								<em>YOGA</em>
							</h1>
							<p className="jjk-chatpage__sub">
								{L(
									lang,
									...
									"An AI-powered guide. Ask for recommendations, categories, prices, ratings, locations, or trip plans around Yogyakarta — answered from 3,399 real places.",
								)}
							</p>
						</div>
					</header>
	...
	window.JJSCREENS = Object.assign(window.JJSCREENS || {}, {
		ChatPanel,
		ChatPage,
	});
})();
